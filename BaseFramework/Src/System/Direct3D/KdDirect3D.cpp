...
// Desc: コンストラクタ
//-----------------------------------------------------------------------------
KdDirect3D::KdDirect3D()
	: m_pDevice(nullptr)
	, m_pDeviceContext(nullptr)
	, m_pGISwapChain(nullptr)
	, m_backBuffer()
	, m_zBuffer()
...
bool KdDirect3D::Init(HWND hWnd, int w, int h, bool deviceDebug, std::string& errMsg)
{
	...
	hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, creationFlags, featureLevels, _countof(featureLevels), D3D11_SDK_VERSION, &m_pDevice, &futureLevel, &m_pDeviceContext);
	if FAILED(hr) {
