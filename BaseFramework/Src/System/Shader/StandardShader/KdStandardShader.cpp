...
void KdStandardShader::DrawMesh(const KdMesh* apMesh, const std::vector<KdMaterial>& aMaterials)
{
	if (apMesh == nullptr) return;
	...
	// 全サブセット
	for (UINT subi = 0; subi < apMesh->GetSubsets().size(); subi++)
	{
		// 面が１枚も無い場合はスキップ
		if (apMesh->GetSubsets()[subi].FaceCount == 0) continue;

		// マテリアルセット
		const KdMaterial& material = aMaterials[apMesh->GetSubsets()[subi].MaterialNo];
