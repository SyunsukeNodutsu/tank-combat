...
public:
	...
	//
	void DrawTex(const KdTexture* tex, const Math::Matrix& matrix, const Math::Color* color = &kWhiteColor)
	{
		if (tex == nullptr) return;
		m_cb0.Work().mTransform = matrix;
		DrawTex(tex, 0, 0, tex->GetInfo().Width, tex->GetInfo().Height, nullptr, color, { 0.5f, 0.5f });
	}
...
private:
	...
	ID3D11InputLayout*				m_VLayout;		// 頂点レイアウト
	ID3D11PixelShader*				m_PS;			// ピクセルシェーダー

	KdConstantBuffer<cbSprite>		m_cb0;			// 定数バッファ
	KdConstantBuffer<cbProjection>	m_cb1;			// 定数バッファ

	bool							m_isBegin;		// Flag
	Math::Matrix					m_mProj2D;		// 2D用正射影行列

	ID3D11DepthStencilState*		m_ds;			// デプスステンシルステート
	ID3D11RasterizerState*			m_rs;			// ラスタライザステート
	ID3D11SamplerState*				m_smp0_Point;	// サンプルステート(_Point)
	ID3D11SamplerState*				m_smp0_Linear;	// サンプルステート(_Linear)
	SaveState						m_saveState;	// ステート記憶/復元用
};