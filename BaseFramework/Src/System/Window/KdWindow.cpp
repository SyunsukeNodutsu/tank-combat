...
//-----------------------------------------------------------------------------
// Name: Create()
// Desc: ウィンドウ生成 ※ウィンドウサイズを固定 URL: http://c-works.gonna.jp/blog/2012/08/post-20.html
//-----------------------------------------------------------------------------
bool KdWindow::Create(int clientWidth, int clientHeight, const std::string& titleName, const std::string& windowClassName)
{
	...
	m_hWnd = CreateWindow(
		...
		WS_OVERLAPPEDWINDOW & ~WS_THICKFRAME & ~WS_MAXIMIZEBOX,	// ウィンドウタイプ
		0,														// ウィンドウの位置（Ｘ座標）
		0,														// ウィンドウの位置（Ｙ座標）						
		clientWidth,											// ウィンドウの幅
...
LRESULT KdWindow::WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		{
			auto result = MessageBoxA(GetWndHandle(), "ゲームを終了しますか？", "確認", MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2);
			if (result == IDYES) { Release(); }
		}
		break;
...
//-----------------------------------------------------------------------------
// Name: SetDesktopCenterWindow()
// Desc: ウィンドウをデスクトップの中央に移動
//-----------------------------------------------------------------------------
BOOL KdWindow::SetDesktopCenterWindow(HWND hWnd)
{
	RECT desktopRect;	// 矩形.デスクトップ
	RECT windowRect;	// 矩形.ウインドウ
	INT posX, posY;		// ウインドウ座標
	INT width, height;	// ウインドウの大きさ
	...
	// 横方向の中央座標軸
	posX = (((desktopRect.right - desktopRect.left) - width) / 2 + desktopRect.left);
	// 縦方向の中央座標軸
	posY = (((desktopRect.bottom - desktopRect.top) - height) / 2 + desktopRect.top);
